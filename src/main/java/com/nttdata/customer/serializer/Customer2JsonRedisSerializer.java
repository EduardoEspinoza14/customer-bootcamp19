 ...
 */
public class Customer2JsonRedisSerializer implements RedisSerializer<CustomerMongo> {
  ...
  @Override
  public byte[] serialize(CustomerMongo customerMongo) throws SerializationException {
    if (customerMongo == null) {
      return EMPTY_ARRAY;
    } else {
      try {
        return this.objectMapper.writeValueAsBytes(customerMongo);
  ...
  @Override
  public CustomerMongo deserialize(byte[] bytes) throws SerializationException {
    if (isEmpty(bytes)) {
      return null;
    } else {
      try {
        ...
        if (hashMap.get("type").equals(CustomerMongo.CUSTOMER_TYPE_1)) {
          type =  this.objectMapper.getTypeFactory().constructType(Person.class);
        } else if (hashMap.get("type").equals(CustomerMongo.CUSTOMER_TYPE_2)) {
          type =  this.objectMapper.getTypeFactory().constructType(Company.class);
        }
